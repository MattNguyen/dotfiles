...
- **git** — `packages/git/gitconfig` has placeholder name/email and includes `~/.gitconfig.local` (untracked). Create `~/.gitconfig.local` on each machine with your real identity:
  ```ini
  [user]
      name = Your Name
